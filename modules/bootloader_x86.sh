...
configure_bootloader_grub() {
  ...
  for k in ${kernel_initrd}; do
    ...
    [ -z "${grub_device}" ] && { error "could not map boot device ${boot_device} to grub device" && return 1 }
  ...
  spawn_chroot "grep -v rootfs /proc/mounts > /etc/mtab" || { error "could not copy /proc/mounts to /etc/mtab" && return 1 }
  [ -z "${bootloader_install_device}" ] && bootloader_install_device="$(get_device_and_partition_from_devnode ${boot} | cut -d '|' -f1)"
  spawn_chroot "grub-install ${bootloader_install_device}" || { error "could not install grub to ${bootloader_install_device}" && return 1 }
}
