...
mount_local_partitions() {
  if [ -z "${localmounts}" ]; then
    warn "no mounts specified. this is a bit unusual, but you're the boss"
  else
...
#      [ -n "${type}" ] && type="-t ${type}"
    ...
    sort -k5 /tmp/install.mount | while read -r mount; do
...
set_timezone() {
  if [ -e "${chroot_dir}/etc/localtime" ]; then
    spawn "rm ${chroot_dir}/etc/localtime" || die "could not remove existing /etc/localtime"
  fi
...
setup_fstab() {
  ...
  for mount in ${localmounts}; do
    ...
    printf "%s\t%s\t%s\t%s\t%s" "${devnode}" "${mountpoint}" "${type}" "${mountopts}" "${dump_pass}" >> "${chroot_dir}/etc/fstab"
  done
  for mount in ${netmounts}; do
    export=$(echo "${mount}" | cut -d '|' -f1)
    type=$(echo "${mount}" | cut -d '|' -f2)
    mountpoint=$(echo "${mount}" | cut -d '|' -f3)
    mountopts=$(echo "${mount}" | cut -d '|' -f4)
    printf "%s\t%s\t%s\t%s\t0 0" "${export}" "${mountpoint}" "${type}" "${mountopts}" >> "${chroot_dir}/etc/fstab"
  done
}
    ...
    for net_device in ${net_devices}; do
      ...
      if [ "${ipdhcp}" = "dhcp" ] || [ "${ipdhcp}" = "DHCP" ]; then
        ...
        if [ "$(ifconfig | grep -C 3 "${device}" | grep inet)" != "" ]; then
          ...
          printf "config_%s=(\"%s netmask %s broadcast %s\")" "${device}" "${IP}" "${NM}" "${BC}" >> "${chroot_dir}/etc/conf.d/net"
	  printf "routes_%s=(\"default via %s\")" "${device}" "${DGW}" >> "${chroot_dir}/etc/conf.d/net"
        else
          debug setup_network_post "No current network config found on ${device}"
        fi
      else
        printf "config_%s=( \"%s\" )\nroutes_%s=( \"default via %s\" )" "${device}" "${ipdhcp}" "${device}" "${gateway}" >> "${chroot_dir}/etc/conf.d/net"
      fi
...
finishing_cleanup() {
  ...
  if [ -e /tmp/install.umount ]; then
    sort -r /tmp/install.umount | while IFS= read -r mnt
    do
